 ...
 * 
 */
class accountAction extends service_Controller_admin
{
    ...
    {
        $model = new service_account_Login_admin();
    ...
    public function loginout()
    {
        $model = new service_account_Login_admin();
        $model->loginOut();
        $redirect = ADMIN_URL . PHP_SELF . '?m=account.login';
    ...
    {
        $username = Input::post( 'username' )->required( '请输入用户名' )->string();
        ...
        $login_data = $model->checkLogin();
        if ( $login_data == false ) {
            throw new ApiException( $model->getErrorMessage() );
        }
        $this->apiReturn( $login_data->uid ); //ajax成功        
    }
    ...
    {
        $model = new service_account_Register_admin();
        $referer_url = $model->getReferer();
        //如果己经登录，不能进入注册页面
        if ( $this->checkLoginStatus() ) {
    ...
    public function register_do()
    {
        ...
        $this->apiReturn( $reg_info ); //ajax成功
    }
    ...
    public function send_verify_code()
    {
        ...
        if ( empty( $refere_url ) || (($_refere_host <> INDEX_URL) && $_refere_host <> ADMIN_URL) ) {
        ...
        //检查用户是否存在
        if ( $sms_type == service_account_Register_admin::sms_type_register && $model->checkMobileRepeat() ) {
    ...
    public function password()
    {
        ...
        $model->setMobile( $mobile );
        $model->setSms_captcha( $sms_captcha );
        $model->setSms_type( service_account_Register_admin::sms_type_forget_password );
        $model->setPassword( $password );
