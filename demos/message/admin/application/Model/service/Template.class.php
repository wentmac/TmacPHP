 ...
 * $Id: Template.class.php 6 2014-09-20 15:13:57Z zhangwentao $
 * http://un.zhuna.cn；
 */
class service_Template_admin extends Model
{
    ...
    public function get_template_info( $template_name, $style_dir )
    {
        ...
        $css_path = $style_dir . $template_name . '/css/style.css';
    ...
    public function recurTemDir( $result, $path_dir, $dir_name, $level, $url )
    {
        ...
        foreach ( $result AS $k => $v ) {
            if ( is_numeric( $k ) ) {                                                
                $file = str_replace( $path_dir . DIRECTORY_SEPARATOR, '', $v[ 'fullname' ] );
    ...
    public function recurDirStyleFile( $path )
    {
        ...
        //得到目录下所有文件
        $cssFiles = scandir( $path . DIRECTORY_SEPARATOR . 'css' );
        $jsFiles = scandir( $path . DIRECTORY_SEPARATOR . 'js' );

        //遍历文件
        foreach ( $cssFiles AS $k => $filename ) {
            //得到完整的名称
            $fullName = $path . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . $filename;
            //$fullName = str_replace( '/', '\\', $fullName );
            if ( !is_dir( $fullName ) ) {
                $filename_ext_array = pathinfo( $filename );
                $filename_ext = $filename_ext_array[ 'extension' ];
                $temp_file = array();
                if ( $filename_ext == 'css' ) {
                    //文件存入临时数组里
                    $temp_file[ 'fullname' ] = $fullName;
                    $temp_file[ 'name' ] = $filename;
                    $temp_file[ 'edittime' ] = date( 'Y-m-d H:i:s', filemtime( $fullName ) );
                    $temp[] = $temp_file;
                }
            }
        }
        //遍历文件
        foreach ( $jsFiles AS $k => $filename ) {
            //得到完整的名称
            $fullName = $path . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . $filename;
            //$fullName = str_replace( '/', '\\', $fullName );
            if ( !is_dir( $fullName ) ) {
                $filename_ext_array = pathinfo( $filename );
                $filename_ext = $filename_ext_array[ 'extension' ];
