...
function _tmac_autoload( $class )
{
    if ( ($appNamePos = strrpos( $class, '_' )) > 0 ) {
        ...
        $classPath = APPS_PATH . $appName . DIRECTORY_SEPARATOR . APPLICATION . DIRECTORY_SEPARATOR . 'Model' . DIRECTORY_SEPARATOR;
...
/**
 * 框架增加namespace的autoload
 * @param type $class
 * @return boolean
 */
function _tmac_namespace_autoload( $class )
{
    if ( false === strpos( $class, '\\' ) ) {
        return true;
    }    
    //namespace autoload
    $app_name = strstr( $class, '\\', true );
    $app_path_name = strstr( $class, '\\' );
    
    $class_path = $app_name . DIRECTORY_SEPARATOR . APPLICATION . DIRECTORY_SEPARATOR . 'Model' . $app_path_name;
    
    $filename = APPS_PATH. str_replace( '\\', DIRECTORY_SEPARATOR, $class_path );
    $filename .= '.class.php';    
    if ( is_file( $filename ) ) {        
        require $filename;
    } else {
...
spl_autoload_register( '_tmac_namespace_autoload' );
