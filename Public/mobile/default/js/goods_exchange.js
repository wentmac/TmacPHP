...
//只显示兑换商品所需的富豪币
$("#item_price,#select_sku_price").text(global_fh_currency_need);
...
var sku_tool = {
	...
	add2cart: function(type) {
		...
		PostField.goods_id = gloabl_goods_id;
		$.post(mobile_url + 'order/exchange_check?goods_id='+gloabl_goods_id, PostField, function(data) {
			if (data.success) {
				if ( data.status == 2 ) {//添加地址
					location.href = mobile_url + 'order/address_add';
					return true;
				}
				$('#select_sku').modal('close');
				location.href = mobile_url + 'order/exchange_confirm?goods_id='+gloabl_goods_id;
			} else {
