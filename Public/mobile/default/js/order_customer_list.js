...
var orderlist = {
	...
	get_order_list: function() {
		...
		$.ajax({
			type: "get",
			url: mobile_url + "member/order.get_customer_list",
			data: {
			...
			success: function(data) {
				if (data.success == true) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						_order_list += '<span class="btn btncancel refundHb"><a href="' + mobile_url + 'member/order.detail?sn=' + list[i].order_sn + '&type=show"><em>查看详细</em></a></span>';
                        _order_list += '<span><a href="' + mobile_url + 'member/order.customer?uid=' + list[i].uid + '"><em>' + list[i].consignee + '</em> <em>[' + list[i].mobile + ']</em> <em>[uid:' + list[i].uid + ']</em></a></span>';
