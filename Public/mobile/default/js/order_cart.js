...
var cart = {
	...
	bind_control: function() {
		...
		$(".cart_seller_wrap").each(function(index) {
			...
			$do_buy.click(function() { //店内结算
        		...
        		var goods_type_platinum_count = 0;
        		var goods_count = 0;
		        ...
		        if ( goods_type_platinum_count > 0 && (goods_type_member_count > 0||goods_type_sale_count > 0||goods_type_member_monopoly_count > 0||goods_type_mall_count > 0||goods_type_other > 0) ) {
		            M._alert( '白金会员商品不能和其他商品一起结算哟~');
		            return false;
		        }
	...
	_shop_price_count: function(shop) {		
		...
		var $use_voucher_money_value_div = $(shop).find('.use_voucher_money_value');
		var $total_amount_div = $(shop).find('.total_amount');
		var $cash_count = $(shop).find('.cash_count');
		var $integral_div = $(shop).find('.integral_div');
		var $item_amount = $(shop).find('.item_amount');//商品总价		
		...
		var use_voucher_money = true;//是否有购物券
		var use_voucher_money_value = 0;//可抵扣的购物券
		...
		$(shop).find('ul.cart_ul li').each(function(i) {
			...
			if ($cart_mask.hasClass('already_mask')) {
				...
				var goods_type = parseInt($(this).attr('data-goods-type'));
                ...
                if ( goods_type != 6 ) {
                	...
                	use_voucher_money_value += price * num;
                }
		...
		//var money = new Number(money).toFixed(2);								
		var use_available_integral = available_integral >= integral_value ? integral_value : available_integral;		
		var total_amount = money;		

        if ( use_integral ) {        	
            total_amount -= use_available_integral;
        }        
        
        var use_voucher_money_value = available_voucher_money >= total_amount ? available_voucher_money : total_amount;                                
        if ( use_voucher_money ) {        	
        	...
        	use_voucher_money_value = 0;
    	}        

        console.log(money);
        console.log(available_integral);
        console.log(use_voucher_money_value);
		console.log(total_amount);
		console.log('-------------');
		var use_available_integral = new Number(use_available_integral);
		var use_voucher_money_value = new Number(use_voucher_money_value);
		var total_amount = new Number(total_amount);	

		
		$use_available_integral_div.text('-¥' + use_available_integral.toFixed(2));
		if ( use_available_integral > 0 ) {
			$integral_div.show();
		} else {
			$integral_div.hide();
		}
		if ( use_voucher_money == true && use_voucher_money_value > 0 ) {
			$use_voucher_money_value_div.text('-¥' + use_voucher_money_value.toFixed(2));
		} else {
			$use_voucher_money_value_div.text('-¥0');
		}
		$total_amount_div.text('¥' + total_amount.toFixed(2));
		$cash_count.text(num_count);				
		$item_amount.text('¥' + new Number(money).toFixed(2));			
	},
