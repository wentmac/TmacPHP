	...
	confirm_click: function() {
		...
		$("#confirm_yes").bind("click", function() {
			...
			if (browser.indexOf('iPhone') > -1 || browser.indexOf('iPad') > -1) {
				MODAL_HTML._confirm("join", "宝身茶提醒-上架成功", join_title, "<a class='am-block' id='confirm_yes'>确定</a>", "<a class='am-block' id='confirm_no'>取消</a>");
			}
		});
				...
				success: function(data) {
					if (data.success == true) {
						...
						if (show_qqqun_status == "1") {
							if ($.cookie('join') == null) {
								MODAL_HTML._confirm("join", "宝身茶提醒-上架成功", "" + join_title + "", "<a id='confirm_yes'>确定</a>", "<a id='confirm_no'>取消</a>");
