...
$server->on( 'message', function (swoole_websocket_server $_server, $frame) {
    ...
    if ( $frame->data == "close" ) {
         ...
         */
        $fd = $frame->fd;
        $_server->tick( 100, function($id) use ($fd, $_server) {
            //$_send = 'client:'.$fd.',PMAG,白银现货,2016-10-28 13:52:50,17.684,17.625,17.7455,17.575';
            $rand = rand(18100,18600);
            $rand_price = $rand/1000;
            $now_time = microtime(TRUE)/1000;
            $last_pmag = "2016-10-28 13:52:50.123,{$rand_price},18.225,18.100,18.600,{$now_time}";
