 ...
 * $Id: member.php 904 2017-02-23 15:30:41Z zhangwentao $
...
//        die;
     ...
     * 新增/修改文章页面
     */
    public function agent_apply_detail()
    {
        $uid = Input::get( 'uid', 0 )->required( '用户不能为空' )->int();
        if ( Filter::getStatus() === false ) {
            $this->redirect( Filter::getFailMessage() );
        ...
        $this->assign( $member );
        //TODO　载入资讯类别添加表单
        $this->V( 'member_agent_apply_detail' );
    }
     ...
     * 新增/修改栏目页面　保存　
     */
    public function agent_apply_save()
    {
        ...
        $verify = Input::post( 'verify', 0 )->int();
        ...
        $memberSettingInfo = $model->getMemberSettingInfoByUid( $uid, 'member_contact' );

        if ( $verify == 1 ) {
            $member_type = service_Member_base::member_type_dfh_agent;
        } else {
            $member_type = service_Member_base::member_type_buyer;
        }

        $entity_Member_base = new \base\entity\Member();
        $entity_Member_base->member_type = $member_type;
        if ( $verify == 1 ) {
            $entity_Member_base->mobile = $memberSettingInfo->member_contact;
        }


        $entity_MemberSetting_base = new \base\entity\MemberSetting();
        $entity_MemberSetting_base->member_type = $member_type;


        $res = $model->updateMemberInfoByAdmin( $entity_Member_base, $entity_MemberSetting_base );
        if ( $res ) {
            $this->redirect( '修改成功', PHP_SELF . '?m=member&member_type=4' );
        }
        die;
        $this->redirect( '修改失败', PHP_SELF . '?m=member.agent_apply_detail&uid=' . $uid );
    }
    ...
    public function save()
    {
        ...
        $agent_province = Input::post( 'province', 0 )->int();
        $agent_city = Input::post( 'city', 0 )->int();
        $agent_district = Input::post( 'district', 0 )->int();
        $dfh_agent_uid = Input::post( 'dfh_agent_uid', 0 )->int();
        ...
        $entity_Member_base = new \base\entity\Member();
        empty( $username ) || $entity_Member_base->username = $username;
        empty( $realname ) || $entity_Member_base->realname = $realname;
        $entity_Member_base->member_type = $member_type;
        ...
        $entity_Member_base->dfh_agent_uid = $dfh_agent_uid;
        ...
        $entity_MemberSetting_base->agent_province = $agent_province;
        $entity_MemberSetting_base->agent_city = $agent_city;
        $entity_MemberSetting_base->agent_district = $agent_district;

        $member_address_model = new service_member_Address_mobile();
        $member_address_model->getFullAgentAddress( $entity_MemberSetting_base );

        $res = $model->updateMemberInfoByAdmin( $entity_Member_base, $entity_MemberSetting_base );
        if ( $res ) {
            $this->redirect( '修改成功', PHP_SELF . '?m=member.detail&uid=' . $uid );
        }
        $this->redirect( '修改失败', PHP_SELF . '?m=member.detail&uid=' . $uid );
    }
