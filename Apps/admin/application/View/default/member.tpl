...
<form name="forms" id="forms" action="{PHP_SELF}?m=member.save" method="post"  onSubmit="return chkForm();">
    <table width="100%" cellspacing="0" cellpadding="0" border="0" class="t_list">
      ...
      <tr {if $memberInfo->member_type<>5}style="display:none"{/if} id="member_type_dfh_agent_uid">
        <td class="td_right">上级代理uid：</td>
        <td class="td_left">
            <input type="text" id="dfh_agent_uid" name="dfh_agent_uid" value="{$memberInfo->dfh_agent_uid}"/>
        </td>
      </tr>      
	  
      <tr {if $memberInfo->member_type<>5}style="display:none"{/if} id="member_type_dfh_agent">
...
<form name="list_form" method="POST" action="{PHP_SELF}?m=member.batch_do&page={$pageCurrent}" onSubmit="return checkDelForm();">
    <table width="100%" cellspacing="0" cellpadding="0" border="0" class="t_list">
      ...
      <td><a href="{PHP_SELF}?m=member.level&uid={$v->uid}">层级排位</a> | <a href="{PHP_SELF}?m=member.detail&uid={$v->uid}">详细</a>{if $member_type==4} | <a href="{PHP_SELF}?m=member.agent_apply_detail&uid={$v->uid}">审核代理商申请</a>{/if}</td>
      </tr>
      <!--{/loop}-->      
