 ...
 * $Id: Save.class.php 1026 2017-04-12 18:57:25Z zhangwentao $
...
class service_settle_Save_admin extends service_settle_Save_base
{
    ...
    public function handleSettleInfo( $entity_Settle_base )
    {
        ...
        $entity_Settle_base->account_type_text = empty( $account_type_array[ $entity_Settle_base->account_type ] ) ? '' : $account_type_array[ $entity_Settle_base->account_type ];
    ...
    public function settleSave( \base\entity\Settle $entity_Settle_base )
    {
        ...
        $member_bill_dao->setWhere( "order_id={$entity_Settle_base->settle_id} AND bill_type=" . service_Member_base::bill_type_expend . " AND bill_expend_type=" . service_Member_base::bill_expend_type_withdrawals );
        $member_bill_info = $member_bill_dao->getInfoByWhere();
        ...
        if ( $entity_Settle_base->settle_status == service_settle_List_base::settle_status_success ) {
            ...
            $wxpay_transfers_model->setDesc( '用户：' . $member_setting_info->nickname . '|大富豪现金奖励￥' . $this->settleInfo->money );
            try {
            ...
            $err_code = isset( $result[ 'err_code' ] ) ? $result[ 'err_code' ] : ''; //错误码
            $err_code_des = isset( $result[ 'err_code_des' ] ) ? $result[ 'err_code_des' ] : ''; //错误描述

            if ( $return_code == 'FAIL' ) {
                Log::getInstance( 'mobile_order_payment_wechatpay_transfers_error' )->write( var_export( $result, TRUE ) . '|' . var_export( $member_setting_info, true ) . '|' . var_export( $this->settleInfo, true ) );
        ...
        if ( $entity_Settle_base->settle_status == service_settle_List_base::settle_status_success ) {
            $settle_money_all = $this->settleInfo->money + $this->settleInfo->commission_charge;
            $entity_MemberSetting_base = new \base\entity\MemberSetting();
            $entity_MemberSetting_base->current_money = new TmacDbExpr( 'current_money-' . $settle_money_all );
            //更新卖家的金钱 商品供应商UID
            $member_setting_dao->setPk( $entity_Settle_base->uid );
            $member_setting_dao->updateByPk( $entity_MemberSetting_base );
            Log::getInstance( 'post_error' )->write( "\r\n" . var_export( $member_setting_info, true ) . '|' . var_export( $entity_MemberSetting_base, true ) . '|' . var_export( $entity_Settle_base, true ) );
            $is_execute = service_Member_base::is_execute_success;
            $message_type = service_PushMessage_base::message_type_settle_success;
            $order_complete = service_Member_base::order_complete_yes;
            $order_finish = service_Member_base::order_finish_yes;
        }
        ...
        $where = "order_id={$entity_Settle_base->settle_id} "
            . " AND uid={$entity_Settle_base->uid}"
            . " AND order_complete=" . service_Member_base::order_complete_no
            . " AND order_finish=" . service_Member_base::order_finish_no
            . " AND bill_type=" . service_Member_base::bill_type_expend
            . " AND bill_expend_type in(" . service_Member_base::bill_expend_type_withdrawals . ',' . service_Member_base::bill_expend_type_withdrawals_commission_charge . ')';
         ...
         */
        if ( $settle_dao->getDb()->isSuccess() && $settle_dao->getDb()->getNumRows() > 0 ) {
            $settle_dao->getDb()->commit();
            //抓虫的，抓到删除
            $member_bill_array = $member_bill_dao->setField( 'member_bill_id' )->getListByWhere();
            $member_service = new service_Member_base();
            foreach ( $member_bill_array as $member_bill ) {
                $member_service->updateMemberBillCurrentMoneyByID( $entity_Settle_base->uid, $member_bill->member_bill_id );
            }

            return true;
        } else {
            $settle_dao->getDb()->rollback();
            Log::getInstance( 'mobile_order_payment_wechatpay_transfers_error' )->write( '提现事务执行失败：'.var_export( $entity_MemberBill_base, TRUE ) . '|' . var_export( $entity_MemberSetting_base, true ) . '|' . var_export( $this->settleInfo, true ) );
            return false;
        }
    }
    ...
    public function settleVerify( $settle_id )
    {
        ...
        $settle_dao = dao_factory_base::getSettleDao();
        $settle_dao->getDb()->startTrans();
        $entity_Settle_base = new \base\entity\Settle();
