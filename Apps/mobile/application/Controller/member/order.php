 ...
 * $Id: order.php 439 2016-10-04 09:47:05Z zhangwentao $
...
class orderAction extends service_Controller_mobile
{
    ...
    {
        //访问权限，只能代理商才能访问此页面
        $check = $this->checkPurview( [ service_Member_base::member_type_dfh_agent ] );
        if ( $check === false ) {
            $this->redirect( $this->getErrorMessage() );
        }

        $uid = Input::get( 'uid', 0 )->int(); //客户的uid
        ...
        $where = $order_list_model->getOrderCustomerListWhere($uid);
        ...
        $fans_nickname = '';
        if ( !empty( $uid ) && $uid <> $this->memberInfo->uid ) {
            $member_model = new service_Member_base();
            $memberInfo = $member_model->getMemberInfoByUid( $uid );
            if ( empty( $memberInfo ) ) {
                $this->redirect( '客户uid不存在' );
            }
            if ( $memberInfo->agent_uid <> $this->memberInfo->uid ) {
                $this->redirect( '没有客户uid=' . $uid . '的权限' );
            }
            $fans_nickname = $memberInfo->nickname;
        ...
        $array[ 'orderList' ] = $res;
        $array[ 'status' ] = $status;
        $array[ 'fans_nickname' ] = $fans_nickname;
...
//        exit;
        $this->V( 'member/order_customer_list' );
    }
    ...
    public function get_customer_list()
    {
        ...
        if ( !empty( $uid ) && $uid <> $this->memberInfo->uid ) {
            ...
            $memberInfo = $member_model->getMemberInfoByUid( $uid );
            if ( empty( $memberInfo ) ) {
                throw new ApiException( '客户uid不存在' );
            }
            if ( $memberInfo->agent_uid <> $this->memberInfo->uid ) {
                throw new ApiException( '没有客户uid=' . $uid . '的权限' );
            }
        }
