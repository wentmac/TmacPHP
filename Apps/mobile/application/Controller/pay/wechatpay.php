 ...
 * $Id: wechatpay.php 795 2016-12-31 16:19:41Z zhangwentao $
...
//        $this->apiReturn($data);die;
        ...
        $order_type = Input::get( 'type', \base\service\Pay::order_type_order )->int(); //支付订单类型（0：订单支付|1：充值支付）
        $callback = Input::get( 'callback', 'callback' )->string();

        if ( $order_type == \base\service\Pay::order_type_pay ) {
            return $this->unifiedorder_pay( $order_sn, $callback );
        }
...
//            print_r( $order );
        ...
        $array[ 'success_url' ] = MOBILE_URL . "order/success?sn={$order_sn}";
        $array[ 'fail_url' ] = MOBILE_URL . 'order/fail?sn=' . $order_sn;
        $this->assign( $array );
     ...
     * @param type $callback
     */
    private function unifiedorder_pay( $pay_id, $callback )
    {
        $order_model = new \base\service\Pay();
        $payInfo = $order_model->getPayInfoById( $pay_id );
        if ( !$payInfo ) {
            throw new ApiException( '没有此充值单', -1, $callback );
        }
        $payInfo instanceof \base\entity\Pay;
        if ( $payInfo->pay_status <> \base\service\Pay::pay_status_unpay ) {
            parent::headerRedirect( DFH_URL . 'member/pay.detail?sn=' . $payInfo->pay_id );
            exit();
            //throw new ApiException( '操作流程错误', -1, $callback );
        }
        $order_id = $payInfo->pay_id;
        //获取支付金额
        $total_fee = $payInfo->pay_amount * 100;
        $fh_currency = $payInfo->pay_amount * \base\service\Pay::exchange_rate;
        $subject = "充值魅力值[{$fh_currency}个][￥{$payInfo->pay_amount}]";
        $body = mb_strcut( $subject, 0, 32, 'utf-8' );
        $out_trade_no = 'pay_' . $order_id;
        ...
        try {
            ...
            $input->SetOut_trade_no( $out_trade_no );
            $input->SetTotal_fee( $total_fee );
            $input->SetTime_start( date( "YmdHis", $payInfo->create_time ) );
            $input->SetTime_expire( date( "YmdHis", $payInfo->create_time + 7200 ) );
            $input->SetGoods_tag( "" );
            $input->SetNotify_url( $this->notify_url );
            $input->SetTrade_type( "JSAPI" );
            $input->SetOpenid( $openId );
...
//            print_r( $order );
        ...
        $array[ 'success_url' ] = DFH_URL . 'dfh.php?m=pay.success&sn=' . $order_id;        
        $array[ 'fail_url' ] = DFH_URL . 'dfh.php?m=pay.fail&sn=' . $order_id;
        $this->assign( $array );
        $this->assign( 'jsApiParameters', $jsApiParameters );
        //$this->apiReturn($jsApiParameters);
        $this->V( 'wechatpay_unifiedorder' );
    ...
    public function notify()
    {
        ...
        //充值订单支付成功回调处理
        if ( strpos( $result[ 'out_trade_no' ], "pay_" ) === 0 ) {
            return $this->notify_pay( $result, $notify_reply );
        }
        //商户自行增加处理流程,
        //例如：更新订单状态
        //例如：数据库操作
     ...
     * @param type $result
     */
    private function notify_pay( $result, $notify_reply )
    {
        //商户自行增加处理流程,
        //例如：更新订单状态
        //例如：数据库操作
        ...
        $pay_id = str_replace( 'pay_', '', $result[ 'out_trade_no' ] );

        $pay_model = new \base\service\Pay();
        $payInfo = $pay_model->getPayInfoById( $pay_id );

        if ( !$payInfo || $payInfo->pay_status == \base\service\Pay::pay_status_pay_success ) {//订单不存在 /订单状态不等于待支付的  已经支付的
            Log::getInstance( 'mobile_order_payment_wechatpay_error' )->write( 'notify_(充值订单不存在 或 充值订单已经支付的):' . " \n" . $xml . "\n" );
            die( $notify_reply->ToXml() );
        }
        $payInfo instanceof \base\entity\Pay;
        ...
        //记录订单支付日志        
        $entity_PayLog_base = new \base\entity\PayLog();
        $entity_PayLog_base->uid = $payInfo->uid;
        $entity_PayLog_base->order_id = 0;
        $entity_PayLog_base->trade_no = $trade_no;
        $entity_PayLog_base->trade_vendor = service_Order_base::trade_vendor_weixin;
        $entity_PayLog_base->trade_fee = $total_fee;
        $entity_PayLog_base->pay_status = -1;
        ...
        $entity_PayLog_base->buyer_email = $buyer_email;
        $entity_PayLog_base->pay_class = service_Order_base::pay_class_web;
        $entity_PayLog_base->pay_id = $pay_id;

        if ( $payInfo->pay_amount == $total_fee ) {
            $pay_model->setTrade_vendor( service_Order_base::trade_vendor_weixin );
            $pay_model->setTrade_no( $trade_no );
            $pay_model->setEntity_PayInfo( $payInfo );
            if ( $pay_model->orderPaySuccess() ) {
                $entity_PayLog_base->order_note = '支付成功';
                $entity_PayLog_base->pay_status = service_Order_base::pay_status_success;
            } else {
                $entity_PayLog_base->order_note = '支付成功 本站订单状态修改失败';
