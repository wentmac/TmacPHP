 ...
 * $Id: error.php 1028 2017-04-13 15:06:10Z zhangwentao $
 * http://shop.weixinshow.com；
 */
//use \mobile\service\Index;
...
class errorAction extends service_Controller_mobile
{
    ...
    {
        //测试跨库事务
        $index = new \mobile\service\Index();
        exit;
        $res = $index->testTransOneDb();
        var_dump($res);
        exit();

        $trend_type_array = Tmac::config( 'trend.trend.trend_type', APP_BASE_NAME );
        print_r($trend_type_array);
        exit();
        $member_level_model = new \crontab\service\MemberLevel();
        //$uid = 21683;
        //$res = $member_level_model->getDFHAgentParentsArray($uid);
        $uid = 1;
        $res = $member_level_model->getAgentParentsArray( $uid );
        echo '<pre>';
        print_r( $res );
        exit();
        ...
        $addarticle = new \base\entity\AddArticle();
        ...
        $addarticle = new \base\entity\AddArticle();
        $addarticle->cat_id = 1;
        $addarticle->article_id = 2;
        $addarticle->content = 2;
        print_r( $addarticle );
        var_dump( $res );
        die;
        ...
        /**
          ...
          $memberInfo = new \base\entity\Member();
          $memberInfo->uid = 75;
