...
<body>
        ...
        <div id="cart_seller_list">
          ...
          <div class="cart_seller_wrap">
            ...
            <ul id="cart_ul_{$v[item_uid]}" class="cart_ul" data-seller="{$v[item_uid]}" data-goods-uid="{$v[goods_uid]}">
              ...
              <li class="cart_li rel" id="{$item->cart_id}" data-cart-id="{$item->cart_id}" data-item-id="{$item->item_id}" data-item-sku="{$item->goods_sku_id}" data-item-price="{$item->item_price}" data-current-num="{$item->item_number}" data-goods-type="$item->goods_type" data-is-integral="$item->is_integral" data-goods-type="{$item->goods_type}">
            ...
            <div class="cart_fix_footer wrap">
              <div class="cart_fix_footer_wrap margin_auto wrap">
                ...
                <div class="cart_fix_footer_money">
                  <p class="cart_fix_footer_notice left">总商品金额:<span class="item_amount">{$v[item_amount]}</span></p>
                  <!--{if $v[use_voucher_money]==1 && $v[use_voucher_money_value]>0 }-->
                  <p class="mycart_money right">购物券抵扣:&nbsp;<span class="use_voucher_money_value i_pri">-¥{$v[use_voucher_money_value]}</span></p>
                  <!--{/if}-->                                  
                </div>

                <!--{if $v[use_integral]==1 && $v[use_available_integral]>0}-->                
                <div class="cart_fix_footer_money integral_div">
                  <p class="mycart_money right">积分抵扣:&nbsp;<span class="use_available_integral i_pri">-¥{$v[use_available_integral]}</span></p>
                </div>                               
                <!--{/if}-->

                <div class="cart_fix_footer_money">
                  <p class="cart_fix_footer_notice left">不含运费</p>                  
                  <p class="mycart_money right">实付总额:&nbsp;<span class="total_amount i_pri">¥{$v[all_amount]}</span></p>                                                    
                </div>                

                <div class="cart_fix_footer_inner rel">
                    <a class="do_buy btnok right" data-sellerid="{$v[item_uid]}">去结算(<em id="cash_count_{$v[item_uid]}" class="cash_count">${echo $num;}</em>)</a>
                </div>
  ...
  var available_voucher_money = '{$available_voucher_money}';
  </script>
<script type="text/javascript" src="{$BASE_V}js/common.js"></script>
  <script type="text/javascript" src="{$BASE_V}js/order_cart.js?v=11"></script>
</body>
