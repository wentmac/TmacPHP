...
<head>
  ...
  <title>我的客户 - {$config[cfg_webname]}</title>
...
.fensi li span,.fensi li em{ margin-left: 0px; display: inline-block;}
.fensi li img { width: 40px; height: 40px;}
.fensi .em_uid{width:15%; float:left}
.fensi .em_nickname{width:30%;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;float:left}
.fensi .em_avatar{width:15%; float:left}
.fensi .em_mobile{width:20%; float:left}
.fensi .em_time{width:15%; }
</style>
...
<body>
  <header id="common_hd" class="c_txt rel">
      ...
      <a id="common_hd_logo" class="t_hide abs common_hd_logo">我的客户</a>
      <h1 class="hd_tle">我的客户</h1>
      <a id="hd_enterShop" class="hide abs" href="{MOBILE_URL}member/home" style="display: block;"> 
        <span id="hd_enterShop_img" class="abs"> <img class="block" src="{$BASE_V}image/common/member_home.png" width="32" height="32" style="display: block;"> </span>会员中心
      </a>
  </header>
        ...
        <div class="affiliate-list text-center">
          <div class="well user-well text-center">
            ...
            <h4 class="color-333 text-center text-overflow">{$memberInfo->nickname}</h4>
              ...
              <div class="fensi">
                <h3>我的客户(${echo count($agent_array);})  |  我的推广积分：{$memberInfo->available_integral} 【可以参加积分换购】</h3>
                <ul>
                {loop $agent_array $value}
                    <li>
                        <a href="{MOBILE_URL}member/order.customer?uid={$value->uid}">
                            <em class="em_uid">{$value->uid}</em>
                            <em class="em_nickname">{$value->nickname}</em>
                            <em class="em_avatar"><span><img src="{$value->member_image_id}"></span></em>
                            <em class="em_mobile">{$value->mobile}</em>
                            <em class="em_time">{$value->reg_time}</em>
                        </a>
                    </li>
                {/loop}                  
                </ul>
