...
class service_account_Register_base extends service_Account_base
{
    ...
    public function sendVerifyCode()
    {
        ...
        $sms_type_effective_time = service_Account_base::sms_type_effective_time;
        switch ( $this->sms_type ) {
            case self::sms_type_register:
            default:
                $message = "您的验证码为{$sms_code}，请于{$sms_type_effective_time}分钟内正确输入，如非本人操作，请忽略此短信";
                $template_id = '199521';
                break;
            ...
            case self::sms_type_bind_verify:
                $message = "您正在修改绑定手机，您的验证码是{$sms_code}，请于{$sms_type_effective_time}分钟内正确输入，请确认是本人操作。";
                $template_id = '199428';
                break;
        }
        ...
        $sms_model->setTemplateId( $template_id );
    ...
    {
        $dao = dao_factory_base::getMemberDao();
        $field = 'uid,member_type,member_class,sum_integral,dfh_agent_uid';
        $dao->setField( $field );
        $dao->setPk( $uid );
        ...
        $return = array(
            ...
            'dfh_agent_uid' => $result->dfh_agent_uid,
        );
        if ( $result == false ) {
            $return[ 'agent_uid' ] = 0;
        } else {
