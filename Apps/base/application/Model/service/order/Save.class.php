 ...
 * $Id: Save.class.php 793 2016-12-31 16:06:55Z zhangwentao $
...
class service_order_Save_base extends service_Order_base
{
    ...
    function setVoucher_money( $voucher_money )
    {
    ...
    public function createOrder()
    {
        ...
        $entity_OrderInfo_base->order_integral_total_price = $order_price[ 'order_integral_total_price' ];
        $entity_OrderInfo_base->voucher_money = $order_price[ 'use_voucher_money_value' ]; //使用的购物券抵扣
        ...
        foreach ( $cart_array AS $cart_info ) {
            ...
            if ( $cart_info->is_integral == service_Goods_base::is_integral_yes && $order_integral_amount > 0 ) {
                ...
                $entity_OrderGoods_base->is_integral = service_Goods_base::is_integral_yes;
                //订单商品使用的积分
        ...
        //order action 表
        $entity_OrderAction_base = new \base\entity\OrderAction();
        ...
        if ( !empty( $entity_OrderInfo_base->order_integral_amount ) ) {
            ...
            $entity_Integral_base->integral = -$entity_OrderInfo_base->order_integral_amount;
            $entity_Integral_base->uid = $entity_OrderInfo_base->uid;
        ...
        //减去购物券
        if ( !empty( $entity_OrderInfo_base->voucher_money ) ) {
            $entity_MemberSetting_base = new \base\entity\MemberSetting();
            $entity_MemberSetting_base->voucher_money = new TmacDbExpr( 'voucher_money-' . $entity_OrderInfo_base->voucher_money );
            $member_setting_dao->setPk( $entity_OrderInfo_base->uid );
            $member_setting_dao->updateByPk( $entity_MemberSetting_base );

            $entity_MemberBill_base = new \base\entity\MemberBill();
            $entity_MemberBill_base->uid = $entity_OrderInfo_base->uid;
            $entity_MemberBill_base->order_id = $order_id;
            $entity_MemberBill_base->money = -$entity_OrderInfo_base->voucher_money;
            ...
            $entity_MemberBill_base->bill_expend_type = service_Member_base::bill_expend_type_coupon_consume; //购物券消费
            $entity_MemberBill_base->bill_note = "[购物券]消费";
    ...
    {
        //modiby @20161215 只有agent_uid为空的时候才能填写
        if ( !empty( $this->memberInfo->agent_uid ) ) {
            return true;
        }
        ...
        if ( !empty( $agent_uid ) ) {
            ...
            $entity_MemberSetting_base = new \base\entity\MemberSetting();
            $member_setting_agent_info = $register_model->insertMemberLevel( $this->memberInfo->uid, $this->agent_uid );
            ...
            $member_setting_dao = dao_factory_base::getMemberSettingDao();
            $member_setting_dao->setWhere( "uid={$this->memberInfo->uid}" )->updateByWhere( $entity_MemberSetting_base );
        }
        return true;
    }
    ...
    {
        if ( $order_type <> service_Order_base::order_type_member && $order_type <> service_Order_base::order_type_member_platinum ) {
            return true;
        }
        ...
        if ( $goods_member_level > $next_member_level && $order_type == service_Order_base::order_type_member ) {
        ...
        $where = "uid={$this->uid} AND order_type=" . $order_type . " AND service_status=" . service_order_Service_base::service_status_waiting_seller_confirm;
        ...
        $where = "uid={$this->uid} AND order_type=" . $order_type . " AND service_status=" . service_order_Service_base::service_status_waiting_buyer_confirm;
        $order_refund_dao->setWhere( $where );
        $res = $order_refund_dao->getInfoByWhere();
        if ( $res ) {
            throw new TmacClassException( '您还有会员商品正在售后退款中,请先处理完退款后才能购买新的会员商品' );
        }
        $where = "uid={$this->uid} AND order_type=" . $order_type . " AND service_status=" . service_order_Service_base::service_status_waiting_customer_confirm;
        $order_refund_dao->setWhere( $where );
        $res = $order_refund_dao->getInfoByWhere();
        if ( $res ) {
            throw new TmacClassException( '您还有会员商品正在售后退款中,请先处理完退款后才能购买新的会员商品' );
    ...
    /**
     * 代金券使用
     * @param \base\entity\OrderInfo $entity_OrderInfo_base
     */
    private function usedCouponCode( \base\entity\OrderInfo $entity_OrderInfo_base )
    {
        if ( empty( $entity_OrderInfo_base->coupon_code ) && empty( $entity_OrderInfo_base->coupon_money ) ) {
            return true;
        }
        $entity_Coupon_base = new \base\entity\Coupon();
    ...
    public function updateGoodsStock( $entity_OrderGoods_base )
    {
        ...
        $entity_Item = new \base\entity\Item();
        ...
        //更新goods表中的原商品的库存和销量
        $entity_Goods_base = new \base\entity\Goods();
        ...
        if ( !empty( $entity_OrderGoods_base->goods_sku_id ) ) {//goods_sku表 
            ...
            $entity_GoodsSku_base = new \base\entity\GoodsSku();
    ...
    public function getOrderAmount()
    {
        ...
        $order_total_price = 0;
        $order_integral_total_price = 0; //参与积分抵扣的商品的原总价 order_goods.item_total_price
        $order_amount = 0;
        ...
        $use_voucher_money = true; //能不能使用购物券
        $use_voucher_money_value = 0; //能使用的购物券
        ...
        foreach ( $cart_array as $cart_info ) {
            ...
            $cart_i++;
            $order_total_price+=$cart_info->item_total_price * $cart_info->item_number; //原价
            $order_amount+=$cart_info->item_price * $cart_info->item_number; //实付价
            ...
            if ( $cart_info->is_integral == service_Goods_base::is_integral_yes ) {
                ...
                $order_integral_total_price += $cart_info->item_total_price * $cart_info->item_number; //参与积分抵扣所有商品的原总价 order_goods.item_total_price
            }
            if ( $cart_info->goods_type <> service_Goods_base::goods_type_member_platinum ) {
                $use_voucher_money = true;
                $use_voucher_money_value += $cart_info->item_price * $cart_info->item_number;
            }
        }
        ...
        if ( $goods_member_level > 0 && $cart_info->goods_type == service_Goods_base::goods_type_member ) {
        ...
        if ( $use_integral ) {//如果有积分抵扣
            $use_available_integral = $this->memberInfo->available_integral >= $integral_value ? round( $integral_value ) : $this->memberInfo->available_integral;
            $total_amount = $order_amount - $use_available_integral;
        } else {
        ...
        //最大可使用的购物券抵扣        
        $use_voucher_money_value += $shipping_fee;
        $use_voucher_money_value = $this->voucher_money >= $total_amount ? $total_amount : $this->voucher_money;
        if ( $use_voucher_money ) {
            //$total_amount += $shipping_fee;
            if ( $total_amount >= $use_voucher_money_value ) {
                $total_amount = $total_amount - $use_voucher_money_value;
            } else {
                $use_voucher_money_value = $total_amount;
                $total_amount = 0;
            }
        }
        //判断代金券
        $return = array(
            ...
            'order_integral_total_price' => $order_integral_total_price, //参与积分抵扣所有的商品的原总价 order_goods.item_total_price            
            'use_voucher_money_value' => $use_voucher_money_value, //实际抵扣的购物券
     ...
     * @return type
     */
    public function saveCustomer( \base\entity\OrderInfo $entity_OrderInfo_base )
    {
        ...
        $entity_Customer_base = new \base\entity\Customer();
