 ...
 * $Id: BuyerHandle.class.php 439 2016-10-04 09:47:05Z zhangwentao $
...
class service_order_BuyerHandle_base extends service_Order_base
{
    ...
    public function cancelOrderInfo()
    {
        ...
        //如果使用了购物券，归还掉
        if ( !empty( $this->orderInfo->voucher_money ) ) {
            $entity_MemberSetting_base = new \base\entity\MemberSetting();
            $entity_MemberSetting_base->voucher_money = new TmacDbExpr( 'voucher_money+' . $this->orderInfo->voucher_money );
            $member_setting_dao->setPk( $this->orderInfo->uid );
            $member_setting_dao->updateByPk( $entity_MemberSetting_base );

            $entity_MemberBill_base = new \base\entity\MemberBill();
            $entity_MemberBill_base->uid = $this->orderInfo->uid;
            $entity_MemberBill_base->order_id = $this->orderInfo->order_id;
            $entity_MemberBill_base->money = $this->orderInfo->voucher_money;
            ...
            $entity_MemberBill_base->bill_expend_type = service_Member_base::bill_expend_type_no; //购物券消费
            $entity_MemberBill_base->bill_note = "[购物券]取消订单购物券退还";
            ...
            $entity_MemberBill_base->bill_uid = $this->orderInfo->uid;
            $entity_MemberBill_base->bill_realname = $this->orderInfo->consignee;
    ...
    public function extendConfirmOrderInfo()
    {
        ...
        if ( ($this->orderInfo->confirm_deadline_time - $this->orderInfo->shipping_time) > (8 * 86400) ) {
            throw new TmacClassException( '只有一次延长收货时间的机会，如果还没有收到货，请联系宝身茶客服' );
        }
        //执行确认收货的事务
        $dao = dao_factory_base::getOrderInfoDao();
        $order_action_dao = dao_factory_base::getOrderActionDao();
