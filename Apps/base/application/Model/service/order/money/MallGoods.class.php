 ...
 * $Id: MallGoods.class.php 708 2016-11-27 14:30:42Z zhangwentao $
...
class service_order_money_MallGoods_base extends service_Order_base
{
    ...
    protected $total_fee;
    //直推奖优惠差值
    protected $commission_fee_diff;
    //排位奖优惠差值
    protected $commission_fee_rank_diff;
    ...
    private function updateMemberSettingMoney()
    {
        ...
        if ( empty( $this->commission_fee_diff ) && empty( $this->commission_fee_rank_diff ) ) {
            return true;
        }
        $entity_OrderGoods_base = new \base\entity\OrderGoods();
        if ( !empty( $this->commission_fee_diff ) ) {
            $goods_commission_fee_diff = round( $this->commission_fee_diff / $this->goods_count, 2 );
            $entity_OrderGoods_base->commission_fee = new TmacDbExpr( "commission_fee+{$goods_commission_fee_diff}" );
        }
        if ( !empty( $this->commission_fee_rank_diff ) ) {
            $goods_commission_fee_rank_diff = round( $this->commission_fee_rank_diff / $this->goods_count, 2 );
            $entity_OrderGoods_base->commission_fee_rank = new TmacDbExpr( "commission_fee_rank+{$goods_commission_fee_rank_diff}" );
        }
    ...
    private function getAgentArray()
    {
        ...
        if ( $this->orderInfo->order_integral_amount > 0 ) {
             ...
             * @20160713改成 由实付减改成总价减
             * 实应付-运费-积分
             */
            //$order_total_price = $this->orderInfo->order_payable_amount - $this->orderInfo->order_integral_amount - $this->orderInfo->shipping_fee; //积分分润要减去积分
            /**
             * @20160728改成 原价总价-使用积分商品的原总价-运费
             */
            $order_total_price = $this->orderInfo->order_total_price - $this->orderInfo->order_integral_total_price; //积分分润要减去积分
        } else {
            //没有用过积分
        ...
        if ( $memberInfo->member_level > 0 ) {//如果自己是会员，没有直推奖            
            if ( $memberInfo->agent_uid > 0 ) {
                ...
                if ( $agentMemberInfo->member_level > $buyerMemberInfo->member_level ) {
                    ...
                    //优惠差值
                    $commission_fee_rank_diff = $this->commission_fee_rank_diff = round( $order_total_price * $rate, 2 );
            ...
            if ( $memberInfo->agent_uid > 0 ) {
                ...
                if ( $agentMemberInfo->member_level > 0 ) {//东家是会员
                    ...
                    $commission_fee_diff = $this->commission_fee_diff = round( $order_total_price * $rate, 2 );
