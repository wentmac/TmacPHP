 ...
 * $Id: Refund.class.php 439 2016-10-04 09:47:05Z zhangwentao $
...
class service_order_Refund_base extends service_Order_base
{
    ...
    public function checkOrderRefundPurview( $order_refund_info )
    {
        ...
        if ( $this->service_status == service_order_Service_base::service_status_success ) {
            ...
            $entity_OrderRefund_base = new \base\entity\OrderRefund();
    ...
    public function executeOrderRefund()
    {
        ...
        } else if ( $this->entity_OrderRefund->order_type == service_Order_base::order_type_member_platinum ) {
            $member_platinum_model = new service_order_money_MemberPlatinum_base();
            $member_platinum_model->setBatch_no( $this->batch_no );
            $member_platinum_model->setRefund_id( $this->refund_id );
            $member_platinum_model->setTrade_no( $this->trade_no );
            $member_platinum_model->setTrade_vendor( $this->trade_vendor );
            $member_platinum_model->setTotal_fee( $this->total_fee );
            $member_platinum_model->setEntity_OrderRefund( $this->entity_OrderRefund );
            $member_platinum_model->refund();
        } else {
            //member_bill 会员账单表更新
    ...
    {
        if ( $this->entity_OrderRefund->order_type == service_Order_base::order_type_member_platinum ) {
            return $this->updateMemberLevelByMemberPlatinum();
        }
        if ( $this->entity_OrderRefund->order_type <> service_Order_base::order_type_member ) {
            return true;
        }
        ...
        $where = "uid={$this->entity_OrderRefund->uid}"
                . " AND order_type=" . $this->entity_OrderRefund->order_type
        ...
        if ( $this->entity_OrderRefund->order_type <> service_Order_base::order_type_member && $memberInfo->member_level_source > $this->entity_OrderRefund->goods_member_level ) {//如果原来的会员级别大于等于当前退款的级别
            //会员级别总数加1
            $member_level_count++;
        }
     ...
     * 更新白金会员商品退款后的退级操作
     */
    private function updateMemberLevelByMemberPlatinum()
    {
        if ( $this->entity_OrderRefund->order_type <> service_Order_base::order_type_member_platinum ) {
            return true;
        }
        ...
        $where = "uid={$this->entity_OrderRefund->uid}"
                ...
                . " AND goods_member_level_refund=" . service_Order_base::goods_member_level_refund_no;
        $order_info_dao->setWhere( $where );
        $order_info_dao->setField( 'order_id,goods_member_level' );
        $order_info_dao->setOrderby( 'goods_member_level DESC' );
        $member_level_array = $order_info_dao->getListByWhere();
        $member_level_count = count( $member_level_array );
        if ( $member_level_count == 0 ) {
            if ( empty( $memberInfo->member_level_source ) ) {
                $member_level = 0;
            } else {
                $member_level = $memberInfo->member_level_source;
            }
        } else {
            $last_goods_member_level = $member_level_array[ 0 ]->goods_member_level;

            $member_level = $last_goods_member_level > $memberInfo->member_level_source ? $last_goods_member_level : $memberInfo->member_level_source;
        }
        $entity_Member_base = new \base\entity\Member();
        $entity_Member_base->member_level = $member_level;
    ...
    public function executeOrderCurrentMoney()
    {
        ...
        $dao->setWhere( $where );
        $dao->setField( 'member_bill_id,uid,order_id,money,bill_type,bill_type_class,bill_expend_type,order_complete,order_finish,bill_class' );
        ...
        foreach ( $res as $member_bill ) {
            ...
            if ( $member_bill->bill_class == service_Member_base::bill_class_money ) {
                $entity_MemberSetting_base = new \base\entity\MemberSetting();
