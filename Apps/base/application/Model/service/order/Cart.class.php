 ...
 * $Id: Cart.class.php 707 2016-11-27 03:09:34Z zhangwentao $
...
class service_order_Cart_base extends service_Order_base
{
    ...
    public function saveCart()
    {
        ...
        if ( $goodsInfo->goods_type == service_Goods_base::goods_type_member || $goodsInfo->goods_type == service_Goods_base::goods_type_member_platinum ) {
            $checkGoodsMemberLevelPurview = $this->checkGoodsMemberPurview( $goodsInfo->goods_member_level, $goodsInfo->goods_type );
        ...
        if ( empty( $this->uid ) ) {
            ...
            $entity_Cart_base = new \base\entity\Cart();
     ...
     * $goods_member_level
     */
    private function checkGoodsMemberPurview( $goods_member_level, $order_type )
    {
        if ( !empty( $this->uid ) ) {
    ...
    public function getCartList()
    {
        ...
        $result_array = array( 'cart_array' => array(), 'cart_amount' => array(), 'use_integral' => array(), 'integral_value' => array(), 'use_voucher_money' => array(), 'voucher_money_value' => array() );
        ...
        if ( $res ) {
            foreach ( $res as $cart_info ) {
                ...
                //判断是否参加积分活动
                if ( !isset( $result_array[ 'use_integral' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] ) ) {
                    $result_array[ 'use_integral' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] = false;
                }
                ...
                //最多使用多少购物券                
                /*
                if ( !isset( $result_array[ 'use_voucher_money' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] ) ) {
                    $result_array[ 'use_voucher_money' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] = false;
                } 
                 * 
                 */                                 
                //if ( $cart_info->goods_type <> service_Goods_base::goods_type_member_platinum ) {
                    $result_array[ 'use_voucher_money' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] = true;
                    if ( isset( $result_array[ 'voucher_money_value' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] ) ) {
                        $result_array[ 'voucher_money_value' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] += $cart_info->item_number * $cart_info->item_price;
                    } else {
                        $result_array[ 'voucher_money_value' ][ $cart_info->goods_uid ][ $cart_info->item_uid ] = $cart_info->item_number * $cart_info->item_price;
                    }
                //}
            ...
            foreach ( $result_array[ 'cart_array' ] AS $goods_uid => $cart_item_array ) {
                foreach ( $cart_item_array as $item_uid => $cart_array ) {
                    $cart_res = array();
                    $integral_value = empty( $result_array[ 'integral_value' ][ $goods_uid ][ $item_uid ] ) ? 0 : $result_array[ 'integral_value' ][ $goods_uid ][ $item_uid ];
                    $use_available_integral = $this->available_integral >= ceil( $integral_value ) ? ceil( $integral_value ) : $this->available_integral;
                    ...
                    if ( $use_integral ) {
                        $total_amount = $total_price - $use_available_integral;
                    } else {
                        $total_amount = $total_price;
                    }
                    $voucher_money_value = empty( $result_array[ 'voucher_money_value' ][ $goods_uid ][ $item_uid ] ) ? '' : $result_array[ 'voucher_money_value' ][ $goods_uid ][ $item_uid ];
                    //最大可用购物券                    
                    $use_voucher_money_value = $this->voucher_money >= $total_amount ? $total_amount : $this->voucher_money;
                    $use_voucher_money = $result_array[ 'use_voucher_money' ][ $goods_uid ][ $item_uid ];
                    ...
                    $cart_res[ 'order_payable_amount' ] = $total_price; //应付总价
                    $cart_res[ 'use_integral' ] = $use_integral; //能不能使用积分 
                    $cart_res[ 'use_available_integral' ] = $use_available_integral; //能使用的积分 
                    $cart_res[ 'use_voucher_money' ] = $use_voucher_money; //能不能使用购物券
                    $cart_res[ 'use_voucher_money_value' ] = $use_voucher_money_value; //能使用的购物券
    ...
    public function getCartListByIdString( $cart_id_string )
    {
        ...
        if ( $res ) {
            foreach ( $res as $key => $value ) {
                ...
                if ( $value->goods_type <> service_Goods_base::goods_type_member_platinum ) {
                    $use_voucher_money = true;
                    $use_voucher_money_value += $value->item_number * $value->item_price;
                }
            }
        }
        ...
        $array = array(
            ...
            'use_voucher_money' => $use_voucher_money, //能不能使用购物券 
            'use_voucher_money_value' => $use_voucher_money_value//能使用的购物券
        );
        return $array;
    }
