 ...
 * $Id: RefundSave.class.php 792 2016-12-31 15:35:43Z zhangwentao $
...
class service_order_RefundSave_base extends service_Order_base
{
    ...
    public function checkPurviewByBuyer()
    {
        ...
        if ( !empty( $this->order_goods_id ) ) {//单个订单商品退款
            ...
            $money = $order_goods_info->item_price * $order_goods_info->item_number + $order_info->shipping_fee - $order_info->coupon_money - $order_goods_info->order_integral_amount - $order_info->voucher_money;
            if ( $this->money > $money ) {
                throw new TmacClassException( '退款金额不能大于订单商品金额' + $money );
            }
            //判断订单结束时间 
        ...
        if ( $this->order_info->order_type == service_Order_base::order_type_exchange ) {
            throw new TmacClassException( '富豪币兑换商品类型的订单不能退款' );
        }
     ...
     * 创建新的退款维权
     */
    public function createOrderRefund( \base\entity\OrderRefund $entity_OrderRefund_base )
    {
        ...
        //处理状态（1：等待卖家处理｜2：等待买家处理｜3：等待宝身茶客服处理｜4：维权处理完成｜5：维权结束/取消）
        ...
        if ( $entity_OrderRefund_base->order_type == service_Order_base::order_type_member || $entity_OrderRefund_base->order_type == service_Order_base::order_type_member_platinum ) {
        ...
        //order_service表
        $entity_OrderService_base = new \base\entity\OrderService();
