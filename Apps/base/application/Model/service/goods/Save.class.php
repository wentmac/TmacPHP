...
class service_goods_Save_base extends Model
{
     ...
     * goods_image
     */
    public function createGoods( \base\entity\Item $entity_Item_base, \base\entity\Goods $entity_Goods_base )
    {
     ...
     * 处理商品的佣金
     */
    protected function handleGoodsCommissionFee( \base\entity\Goods $entity_Goods_base )
    {
    ...
    public function modifyGoods( \base\entity\Item $entity_Item_base, \base\entity\Goods $entity_Goods_base )
    {
        $item_dao = dao_factory_base::getItemDao();
        $goods_dao = dao_factory_base::getGoodsDao();
    ...
    public function _saveGoodsImage( $goods_id )
    {
        ...
        if ( empty( $this->goods_id ) ) {
            foreach ( $this->goods_image_array AS $goods_image_id ) {
                $entity_GoodsImage_base = new \base\entity\GoodsImage();
        ...
        if ( empty( $this->goods_id ) ) {
            foreach ( $this->goods_spec_array AS $spec_id => $spec_value_array ) {
                foreach ( $spec_value_array AS $spec_value_id ) {
                    ...
                    $entity_GoodsSpec_base = new \base\entity\GoodsSpec();
    ...
    public function _saveItemCategoryMap( $item_id, $goods_id )
    {
        ...
        if ( empty( $this->item_id ) ) {
            foreach ( $this->item_cat_id_array AS $item_cat_id ) {
                $entity_ItemCategoryMap_base = new \base\entity\ItemCategoryMap();
    ...
    public function _saveGoodsCategoryMap( $goods_id )
    {
        ...
        if ( empty( $this->goods_id ) ) {
            foreach ( $this->goods_cat_id_array AS $goods_cat_id ) {
                $entity_GoodsCategoryMap_base = new \base\entity\GoodsCategoryMap();
    ...
    private function updateCategoryItemCount( $item_cat_id )
    {
        ...
        $entity_ItemCategory_base = new \base\entity\ItemCategory();
    ...
    private function updateCategoryGoodsCount( $goods_cat_id )
    {
        ...
        $entity_GoodsCategory_base = new \base\entity\GoodsCategory();
    ...
    /**
     * 处理原价/实际销价
     * @param \base\entity\Goods $entity_Goods_base
     * @return \entity_Goods_base
     */
    protected function handelPromotePriceDifference( \base\entity\Goods $entity_Goods_base )
    {
