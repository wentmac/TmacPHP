 ...
 */
class service_oauth_Weixin_base extends service_Oauth_base implements service_oauth_Interface_base
{
    ...
    public function weixinRegister( $access_token_result, $userinfo )
    {
        ...
        $agent_uid_cookie = Input::cookie( 'agent_uid', 0 )->int();
        if ( $agent_uid_cookie ) {
            $agent_info = $register_model->getUidByUid( $agent_uid_cookie );
            $agent_uid = empty( $agent_info ) ? 0 : $agent_info[ 'agent_uid' ];
        }

        if ( $agent_uid ) {
            $register_model->setOpenid( $userinfo->openid );
            $register_model->setEventKey( 'qrscene_'.$agent_uid );
            try {
