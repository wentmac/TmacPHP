 ...
 * $Id: Save.class.php 1025 2017-04-12 18:49:38Z zhangwentao $
...
class service_settle_Save_base extends service_Model_base
{
     ...
     * 最小提现金额
     */
    const min_setting_money = 2;
    const max_setting_money = 20000;
     ...
     * 取手续费
     */
    private function getFee( $settle_money )
    {
        //百分之一
        $fee = round( $settle_money * 0.01, 2 );
        $settle_fee = $fee > 2 ? 2 : $fee;
        return [ 'settle_money' => $settle_money - $settle_fee, 'fee' => $settle_fee ];
    }
    ...
    public function createSettle()
    {
        ...
        if ( $this->memberInfo->locked_type == service_Member_base::locked_type_settle ) {
            throw new TmacClassException( '您的账户出现异常，暂时无法提现。请联系宝身茶客服解决！' );
        }
        //判断金额
        ...
        //不能大于2万
        if ( $this->money > self::max_setting_money ) {
            throw new TmacClassException( '提现金额不能大于：￥' . self::max_setting_money . '元' );
        }
        //检测是否有没有处理过的申请
        $this->checkIsUnHandleSettle();

        //取手续费
        $money_array = $this->getFee( $this->money );
        ...
        $entity_MemberBill_base->order_id = 0; //当是提现时。order_id放 settle_id
        $entity_MemberBill_base->money = -$money_array[ 'settle_money' ];
        ...
        $entity_MemberBill_base->bill_note = "申请提现:{$this->money}元，实际到账:{$money_array['settle_money']}元,手续费:{$money_array['fee']}元";
        ...
        $entity_Settle_base->money = $money_array[ 'settle_money' ];
        $entity_Settle_base->commission_charge = $money_array[ 'fee' ];
        ...
        $settle_id = $dao->insert( $entity_Settle_base );

        //更新member_bill表中的settle_id
        $entity_MemberBill_base->order_id = $settle_id; //当是提现时。order_id放 settle_id
        $member_bill_dao->setPk( $member_bill_id );
        $member_bill_dao->updateByPk( $entity_MemberBill_base );

        $entity_MemberBill_base = new \base\entity\MemberBill();
        $entity_MemberBill_base->uid = $this->uid;
        $entity_MemberBill_base->order_id = $settle_id; //当是提现时。order_id放 settle_id
        $entity_MemberBill_base->money = -$money_array[ 'fee' ];
        $entity_MemberBill_base->bill_type = service_Member_base::bill_type_expend;
        $entity_MemberBill_base->bill_type_class = service_Member_base::bill_type_class_no; //提现没有金额流入
        $entity_MemberBill_base->bill_expend_type = service_Member_base::bill_expend_type_withdrawals_commission_charge;
        $entity_MemberBill_base->bill_note = "申请提现:{$this->money}元，手续费:{$money_array['fee']}元";
        $entity_MemberBill_base->bill_time = $this->now;
        $entity_MemberBill_base->is_execute = 0;
        $entity_MemberBill_base->execute_time = 0;
        $entity_MemberBill_base->trade_vendor = '';
